...
- 更新环境 `conda env update --file environment.yml --prune`
...
- 初始化 `alembic init alembic --template async`
- 生成更新脚本 `alembic revision --autogenerate -m "数据库更改"`
- 更新数据库 `alembic upgrade head`
- 适配自定义类型
    - 在 `alembic/script.py.mako` 里添加 `import src`

## MathType

- 重新试用30天：删除注册表的`计算机\HKEY_CURRENT_USER\SOFTWARE\Install Options`中的`Options6.9`